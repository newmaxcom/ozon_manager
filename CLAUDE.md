...
| OzonQueueModel | `onec_supply.ozon_supplies_queue` | composite (`doc_number`, `order_numbers`, `account`) | очередь поставок: `onec_prefix` (например `ТМ`), `plan_date`, `items` (JSONB), `macrolocal_cluster_id`, цепочка `draft_id → order_id → supply_id`, `order_number`, `data_filling_deadline_utc`, `timeslot_from/to`, `state`, `is_error/error_text`, `is_for_push` (флаг для 1С). Sync при старте. |
| StatusModel | `ozon_supply.supply_status` | composite (`company`, `supply_id`) | read-only: `state`, `status`, `state_updated_date`, `bundle_id`. Наполняет [ozon_parser](../ozon_parser/CLAUDE.md), читаем в dashboard как источник state (приоритет над live кэшем queue.state). |
